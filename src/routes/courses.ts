...
route.post('/:id1/batches',(req:Request,res:Response)=>{
    ...
    Batch.create({batchName:batchName,courseId:req.params.id1})
    .then((addedBatch:any)=>{
        res.json({success:true})
    })
    .catch((err:Error)=>res.send({success:false}))
...
route.post('/:id1/batches/:id2/lectures',(req:Request,res:Response)=>{
    ...
    .then((subjectId:any)=>{
        Lecture.create({lectureName:lectureName,subjectId:subjectId.subjectId,teacherId:teacherId,batchId:batchId})
...
route.post('/:id1/batches/:id2/bulklectures',(req:Request,res:Response)=>{

    let allLectures=req.body.lectures
    for(let lecture of allLectures)
    {
        let batchId=req.params.id2;
        let lectureName=lecture.lectureName;
        let teacherId=lecture.teacherId;
        ...
        .then((subject:any)=>{
            ...
            Lecture.create({lectureName:lectureName,subjectId:subject.subjectId,teacherId:teacherId,batchId:batchId})
            .then((createdlecture:any)=>{
                // res.send({success:true})
            })
            .catch((err:Error)=>{res.send({success:false})})
